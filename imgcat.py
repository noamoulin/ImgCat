...
class Plan:
    def __init__(self, planches, void):
        ...
        matrix = [[image_at((x, y), planches, cv2.resize(void, dsize = (0,0), fx = SIZE, fy = SIZE)) for x in range(min(abss), max(abss) + 1)] for y in range(min(ords), max(ords) + 1)]
        self.image = cv2.vconcat(list(map(lambda row : cv2.hconcat(row), matrix)))

plan = Plan(list(map(lambda path : Planche(path), list(filter(lambda elm : not elm in (os.path.realpath(__file__).split('/')[-1], BACKGROUND_PATH), os.listdir())))), cv2.imread(BACKGROUND_PATH))
